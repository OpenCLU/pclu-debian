...
#endif
	size = top - low.num + 1;
	size = size * 2; /* dwc 10/15/21 wordvec elements are now 4 bytes rather than 2 bytes */
	if (size <= 0) signal(ERR_ok);
	for (;;) {
...
_chan *ch  = (_chan *)chref.ref;
	...
	if (bv.str->size == 0) signal(ERR_bounds);
	while (1) {
		/* result = read(ch->rd.num, bv.str->data, bv.str->size); beef up read count... */
		result = read(ch->rd.num, bv.str->data, 2*bv.str->size);
		if (result == -1 && errno == EINTR) continue;
		if (result >= 0) break;
		elist[0] = _unix_erstr(errno);
		signal(ERR_not_possible);
